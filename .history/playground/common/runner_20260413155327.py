...
class BaseRunner(ABC):
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        self.num_envs_override = getattr(args, "num_envs", None)
    ...
    def train(self) -> None:
        ...
        if self.num_envs_override is not None:
            ...
            num_envs = self.num_envs_override
            # num_minibatches must divide evenly across devices
            base_minibatches = self.ppo_training_params.get("num_minibatches", 32)
            self.ppo_training_params["num_envs"] = num_envs
            self.ppo_training_params["num_minibatches"] = max(base_minibatches, base_minibatches * (num_devices // max(1, jax.device_count() // num_devices)))
            print(f"[multi-GPU] devices={num_devices}, num_envs={num_envs}, num_minibatches={self.ppo_training_params['num_minibatches']}")
